...
namespace EVP {
    ...
    public class WheelData {
        ...
        public PhysicMaterial
            lastPhysicMaterial
                = new PhysicMaterial(); // A new physic material ensures the first iteration to match the changes.

        public RaycastHit
            rayHit; // Result of raycast for visual wheels. Used for precise positioning (i.e. tire marks).
        ...
        public float
            downforceRatio
                = 0.0f; // A relative measure of the amout of weight supported by each wheel. Will be 1.0 in a balanced car at rest.
    }
    ...
    public class VehicleController : MonoBehaviour {
        ...
        [ BoxGroup("Vehicle Input") ] [ Range(- 1, 1) ]
        public float steerInput = 0.0f;

        [ BoxGroup("Vehicle Input") ] [ Range(-1, 1) ]
        public float throttleInput = 0.0f;

        [ BoxGroup("Vehicle Input") ] [ Range(0, 1) ]
        ...
        void UpdateSuspension(WheelData wd) {
            ...
            if (wd.grounded) {
                wd.suspensionCompression = 1.0f -
                                           ( - wd.transform.InverseTransformPoint(wd.hit.point).y -
                                             wd.collider.radius ) / wd.collider.suspensionDistance;
        ...
        void ComputeTireForces(WheelData wd) {
            ...
            if (wd.grounded) {
                ...
                wd.dragForce = - ( forceMagnitude * wd.localVelocity.magnitude * groundDrag * 0.001f ) *
                               wd.localVelocity;
            }
            else {
            ...
            if (wd.isBraking)
                slip = Mathf.Clamp(- slipRatio * wd.localVelocity.y * wheelBrakeRatio, - wheelBrakeSlip,
                    wheelBrakeSlip);
            else
        ...
        public float GetSpeed => maxSpeedForward * 0.2778f;

        //set { speedKMH = value; }
        public float SetSpeed {
            set => maxSpeedForward = value * 0.2778f;
        }
        ...
        void ComputeBrakeValues(WheelData wd,
                                BrakeMode mode,
                                float     maxSlip,
                                float     maxRatio,
                                out float brakeSlip,
        ...
        void UpdateTransform(WheelData wd, float deltaTime) {
            if (wd.wheel.wheelTransform != null || wd.wheel.caliperTransform != null) {
                ...
                if (wheelPositionMode == PositionMode.Fast) {
                    ...
                    bool collided = Physics.Raycast(wd.origin, - wd.transform.up, out wd.rayHit,
                        ( wd.collider.suspensionDistance + wd.collider.radius ), Physics.DefaultRaycastLayers,
                        QueryTriggerInteraction.Ignore);
#else
				bool collided =
 Physics.Raycast(wd.origin, -wd.transform.up, out wd.rayHit, (wd.collider.suspensionDistance + wd.collider.radius), Physics.DefaultRaycastLayers);
#endif
                    ...
                    if (collided) {
                        ...
                        if (Physics.GetIgnoreLayerCollision(wd.collider.gameObject.layer,
                                wd.rayHit.collider.gameObject.layer)) {
                            elongation = wd.collider.suspensionDistance * ( 1.0f - wd.suspensionCompression ) +
                                         wd.collider.radius * 0.05f;
                            wd.rayHit.point  = wd.hit.point;
                            wd.rayHit.normal = wd.hit.normal;
        ...
        void UpdateGroundMaterialCached(PhysicMaterial     colliderMaterial,
                                        ref PhysicMaterial cachedMaterial,
        ...
        public float SphereRaycastOthers(Vector3 origin,
                                         Vector3 direction,
                                         float   radius,
                                         float   maxDistance,
                ...
                if (w.wheelCollider != null) {
                    middlePoint += transform
                                  .InverseTransformPoint(
                                       w.wheelCollider.transform.TransformPoint(w.wheelCollider.center)).z;
                    middleCount ++;
                }
            }
        ...
        void ProcessContacts(Collision col, bool forceImpact) {
            ...
            foreach (ContactPoint contact in col.contacts) {
                ...
                if (dragRatio < - impactThreeshold ||
                    forceImpact && col.relativeVelocity.sqrMagnitude > sqrImpactSpeed) {
                    // Impact
        ...
        void HandleImpacts() {
            ...
            if (Time.time - m_lastImpactTime >= impactInterval && m_sumImpactCount > 0) {
                ...
                m_lastImpactTime = Time.time + impactInterval *
